/****************************************************************************
** Meta object code from reading C++ file 'connectioncheck.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../RedminePlugin/redmineclinet/connectioncheck.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'connectioncheck.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_ConnectionCheck_t {
    QByteArrayData data[9];
    char stringdata0[96];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_ConnectionCheck_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_ConnectionCheck_t qt_meta_stringdata_ConnectionCheck = {
    {
QT_MOC_LITERAL(0, 0, 15), // "ConnectionCheck"
QT_MOC_LITERAL(1, 16, 13), // "sigReplyError"
QT_MOC_LITERAL(2, 30, 0), // ""
QT_MOC_LITERAL(3, 31, 11), // "ReplayError"
QT_MOC_LITERAL(4, 43, 3), // "err"
QT_MOC_LITERAL(5, 47, 10), // "sigReplyOk"
QT_MOC_LITERAL(6, 58, 16), // "sltReplyFinished"
QT_MOC_LITERAL(7, 75, 14), // "QNetworkReply*"
QT_MOC_LITERAL(8, 90, 5) // "reply"

    },
    "ConnectionCheck\0sigReplyError\0\0"
    "ReplayError\0err\0sigReplyOk\0sltReplyFinished\0"
    "QNetworkReply*\0reply"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_ConnectionCheck[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   29,    2, 0x06 /* Public */,
       5,    0,   32,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       6,    1,   33,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 7,    8,

       0        // eod
};

void ConnectionCheck::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        ConnectionCheck *_t = static_cast<ConnectionCheck *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->sigReplyError((*reinterpret_cast< ReplayError(*)>(_a[1]))); break;
        case 1: _t->sigReplyOk(); break;
        case 2: _t->sltReplyFinished((*reinterpret_cast< QNetworkReply*(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        switch (_id) {
        default: *reinterpret_cast<int*>(_a[0]) = -1; break;
        case 2:
            switch (*reinterpret_cast<int*>(_a[1])) {
            default: *reinterpret_cast<int*>(_a[0]) = -1; break;
            case 0:
                *reinterpret_cast<int*>(_a[0]) = qRegisterMetaType< QNetworkReply* >(); break;
            }
            break;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (ConnectionCheck::*_t)(ReplayError );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ConnectionCheck::sigReplyError)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (ConnectionCheck::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ConnectionCheck::sigReplyOk)) {
                *result = 1;
                return;
            }
        }
    }
}

const QMetaObject ConnectionCheck::staticMetaObject = {
    { &IBase::staticMetaObject, qt_meta_stringdata_ConnectionCheck.data,
      qt_meta_data_ConnectionCheck,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *ConnectionCheck::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *ConnectionCheck::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_ConnectionCheck.stringdata0))
        return static_cast<void*>(const_cast< ConnectionCheck*>(this));
    return IBase::qt_metacast(_clname);
}

int ConnectionCheck::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = IBase::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void ConnectionCheck::sigReplyError(ReplayError _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void ConnectionCheck::sigReplyOk()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
